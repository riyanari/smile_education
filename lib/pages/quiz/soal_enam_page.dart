...
import 'package:smile_education/pages/quiz/soal_tujuh_page.dart';
import 'package:smile_education/theme/theme.dart';

import '../home_page.dart';

class SoalEnamPage extends StatefulWidget {
  const SoalEnamPage({super.key});

  @override
  State<SoalEnamPage> createState() => _SoalEnamPageState();
}

class _SoalEnamPageState extends State<SoalEnamPage> {
  String nama = '';
  String totalPoin = '';
  ...
  void _playSoal6() async {
    await _audioPlayer.play(AssetSource('kuis/soal6.mp3'));
  }

  void _play6A() async {
    await _audioPlayer.play(AssetSource('kuis/6A.mp3'));
  }

  void _play6B() async {
    await _audioPlayer.play(AssetSource('kuis/6B.mp3'));
  }
  ...
  Future<void> _getTotalPoinFromSharedPrefs() async {
    ...
    setState(() {
      totalPoin = (point1+point2+point3+point4+point5).toString();
    });
  }
  ...
  Future<void> _savePoints(int point) async {
    SharedPreferences prefs = await SharedPreferences.getInstance();
    await prefs.setInt('point6', point);
  }
  ...
  void _cekJawaban() {
    ...
    AwesomeDialog(
      ...
      btnOkOnPress: () {
        Navigator.push(
            context,
            MaterialPageRoute(builder: (context) => const SoalTujuhPage())
        );
      },
              ...
              children: [
                ...
                Padding(
                  ...
                  child: GestureDetector(
                    onTap: (){
                      if (label == "A") {
                        _play6A(); // Mainkan audio untuk jawaban A
                      } else if (label == "B") {
                        _play6B(); // Mainkan audio untuk jawaban B
                      }
                    },
                    child: Image.asset(
                      'assets/ic_sound.png',
    ...
    Widget soal(){
      return Column(
        children: [
                  ...
                  child: Row(
                    children: [
                      ...
                      Expanded(  // Wrap the Text widget with Expanded to allow it to take available space
                        child: Text(
                          "Apa tanda dan gejala menstruasi?",
                          style: primaryTextStyle.copyWith(
                            fontSize: 14,
                            fontWeight: semiBold,
                          ),
          ...
          answerContainer("Keram perut, perubahan suasana hati, pendarahan vagina, payudara terasa nyeri", "A"),
          const SizedBox(height: 14,),
          answerContainer("Pendarahan hebat", "B"),
        ],
      );
    }
