...
import 'package:smile_education/pages/quiz/soal_tiga_page.dart';
import 'package:smile_education/theme/theme.dart';

import '../home_page.dart';

class SoalDuaPage extends StatefulWidget {
  const SoalDuaPage({super.key});

  @override
  State<SoalDuaPage> createState() => _SoalDuaPageState();
}

class _SoalDuaPageState extends State<SoalDuaPage> {
  String nama = '';
  String totalPoin = '';
  ...
  void _playSoal2() async {
    await _audioPlayer.play(AssetSource('kuis/soal2.mp3'));
  }

  void _play2A() async {
    await _audioPlayer.play(AssetSource('kuis/2A.mp3'));
  }

  void _play2B() async {
    await _audioPlayer.play(AssetSource('kuis/2B.mp3'));
  }
  ...
  Future<void> _getTotalPoinFromSharedPrefs() async {
    SharedPreferences prefs = await SharedPreferences.getInstance();
    int point1 = prefs.getInt('point1') ?? 0; // Ambil point1, jika tidak ada, anggap 0
    // int point2 = prefs.getInt('point2') ?? 0; // Ambil point2, jika tidak ada, anggap 0
    setState(() {
      totalPoin = (point1).toString(); // Jumlahkan point1 dan point2
    });
  }
  ...
  Future<void> _savePoints(int point) async {
    SharedPreferences prefs = await SharedPreferences.getInstance();
    await prefs.setInt('point2', point);
  }

  void _selectAnswer(String label) {
    setState(() {
      selectedAnswer = label; // Simpan label yang dipilih
    });
  }
  ...
  void _cekJawaban() {
    ...
    AwesomeDialog(
      ...
      btnOkOnPress: () {
        Navigator.push(
            context,
            MaterialPageRoute(builder: (context) => SoalTigaPage())
        );
      },
              ...
              children: [
                ...
                Padding(
                  ...
                  child: GestureDetector(
                    onTap: (){
                      if (label == "A") {
                        _play2A(); // Mainkan audio untuk jawaban A
                      } else if (label == "B") {
                        _play2B(); // Mainkan audio untuk jawaban B
                      }
                    },
                    child: Image.asset(
                      'assets/ic_sound.png',
    ...
    Widget soal(){
      return Column(
        children: [
          ...
          answerContainer("Proses biologis yang dilakukan oleh makhluk hidup untuk menghasilkan keturunan", "A"),
          const SizedBox(height: 14,),
          answerContainer("Keluarnya darah dari vagina yang terjadi sebagai dampak dari siklus bulanan", "B"),
        ],
      );
    }
