...
import 'package:smile_education/pages/quiz/soal_lima_page.dart';
import 'package:smile_education/theme/theme.dart';

import '../home_page.dart';

class SoalEmpatPage extends StatefulWidget {
  const SoalEmpatPage({super.key});

  @override
  State<SoalEmpatPage> createState() => _SoalEmpatPageState();
}

class _SoalEmpatPageState extends State<SoalEmpatPage> {
  String nama = '';
  String totalPoin = '';
  ...
  void _playSoal4() async {
    await _audioPlayer.play(AssetSource('kuis/soal4.mp3'));
  }

  void _play4A() async {
    await _audioPlayer.play(AssetSource('kuis/4A.mp3'));
  }

  void _play4B() async {
    await _audioPlayer.play(AssetSource('kuis/4B.mp3'));
  }
  ...
  Future<void> _getTotalPoinFromSharedPrefs() async {
    ...
    setState(() {
      totalPoin = (point1+point2+point3).toString();
    });
  }
  ...
  Future<void> _savePoints(int point) async {
    SharedPreferences prefs = await SharedPreferences.getInstance();
    await prefs.setInt('point4', point);
  }
  ...
  void _cekJawaban() {
    ...
    AwesomeDialog(
      ...
      btnOkOnPress: () {
        Navigator.push(
            context,
            MaterialPageRoute(builder: (context) => const SoalLimaPage())
        );
      },
              ...
              children: [
                ...
                Padding(
                  ...
                  child: GestureDetector(
                    onTap: (){
                      if (label == "A") {
                        _play4A(); // Mainkan audio untuk jawaban A
                      } else if (label == "B") {
                        _play4B(); // Mainkan audio untuk jawaban B
                      }
                    },
                    child: Image.asset(
                      'assets/ic_sound.png',
    ...
    Widget soal(){
      return Column(
        children: [
                  ...
                  child: Row(
                    children: [
                      ...
                      Expanded(  // Wrap the Text widget with Expanded to allow it to take available space
                        child: Text(
                          "Apakah wanita mengalami menstruasi selama hidupnya?",
                          style: primaryTextStyle.copyWith(
                            fontSize: 14,
                            fontWeight: semiBold,
                          ),
          ...
          answerContainer("Ya, sepanjang hidupnya", "A"),
          const SizedBox(height: 14,),
          answerContainer("Tidak, karena wanita akan mengalami menopause", "B"),
        ],
      );
    }
