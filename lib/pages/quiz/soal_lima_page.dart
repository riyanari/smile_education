...
import 'package:smile_education/pages/quiz/soal_enam_page.dart';
import 'package:smile_education/theme/theme.dart';

import '../home_page.dart';

class SoalLimaPage extends StatefulWidget {
  const SoalLimaPage({super.key});

  @override
  State<SoalLimaPage> createState() => _SoalLimaPageState();
}

class _SoalLimaPageState extends State<SoalLimaPage> {
  String nama = '';
  String totalPoin = '';
  ...
  void _playSoal5() async {
    await _audioPlayer.play(AssetSource('kuis/soal5.mp3'));
  }

  void _play5A() async {
    await _audioPlayer.play(AssetSource('kuis/5A.mp3'));
  }

  void _play5B() async {
    await _audioPlayer.play(AssetSource('kuis/5B.mp3'));
  }
  ...
  Future<void> _getTotalPoinFromSharedPrefs() async {
    ...
    setState(() {
      totalPoin = (point1+point2+point3+point4).toString();
    });
  }
  ...
  Future<void> _savePoints(int point) async {
    SharedPreferences prefs = await SharedPreferences.getInstance();
    await prefs.setInt('point5', point);
  }
  ...
  void _cekJawaban() {
    ...
    AwesomeDialog(
      ...
      btnOkOnPress: () {
        Navigator.push(
            context,
            MaterialPageRoute(builder: (context) => const SoalEnamPage())
        );
      },
              ...
              children: [
                ...
                Padding(
                  ...
                  child: GestureDetector(
                    onTap: (){
                      if (label == "A") {
                        _play5A(); // Mainkan audio untuk jawaban A
                      } else if (label == "B") {
                        _play5B(); // Mainkan audio untuk jawaban B
                      }
                    },
                    child: Image.asset(
                      'assets/ic_sound.png',
    ...
    Widget soal(){
      return Column(
        children: [
                  ...
                  child: Row(
                    children: [
                      ...
                      Expanded(  // Wrap the Text widget with Expanded to allow it to take available space
                        child: Text(
                          "Berapa lama wanita mengalami menstruasi?",
                          style: primaryTextStyle.copyWith(
                            fontSize: 14,
                            fontWeight: semiBold,
                          ),
          ...
          answerContainer("3-7 hari", "A"),
          const SizedBox(height: 14,),
          answerContainer("20-30 hari", "B"),
        ],
      );
    }
