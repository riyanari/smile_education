...
import 'package:smile_education/pages/quiz/soal_delapan_page.dart';
import 'package:smile_education/theme/theme.dart';

import '../home_page.dart';

class SoalTujuhPage extends StatefulWidget {
  const SoalTujuhPage({super.key});

  @override
  State<SoalTujuhPage> createState() => _SoalTujuhPageState();
}

class _SoalTujuhPageState extends State<SoalTujuhPage> {
  String nama = '';
  String totalPoin = '';
  ...
  void _playSoal7() async {
    await _audioPlayer.play(AssetSource('kuis/soal7.mp3'));
  }

  void _play7A() async {
    await _audioPlayer.play(AssetSource('kuis/7A.mp3'));
  }

  void _play7B() async {
    await _audioPlayer.play(AssetSource('kuis/7B.mp3'));
  }
  ...
  Future<void> _getTotalPoinFromSharedPrefs() async {
    ...
    setState(() {
      totalPoin = (point1+point2+point3+point4+point5+point6).toString();
    });
  }
  ...
  Future<void> _savePoints(int point) async {
    SharedPreferences prefs = await SharedPreferences.getInstance();
    await prefs.setInt('point7', point);
  }
  ...
  void _cekJawaban() {
    ...
    AwesomeDialog(
      ...
      btnOkOnPress: () {
        Navigator.push(
            context,
            MaterialPageRoute(builder: (context) => const SoalDelapanPage())
        );
      },
              ...
              children: [
                ...
                Padding(
                  ...
                  child: GestureDetector(
                    onTap: (){
                      if (label == "A") {
                        _play7A(); // Mainkan audio untuk jawaban A
                      } else if (label == "B") {
                        _play7B(); // Mainkan audio untuk jawaban B
                      }
                    },
                    child: Image.asset(
                      'assets/ic_sound.png',
    ...
    Widget soal(){
      return Column(
        children: [
                  ...
                  child: Row(
                    children: [
                      ...
                      Expanded(  // Wrap the Text widget with Expanded to allow it to take available space
                        child: Text(
                          "Apa yang dianjurkan pada saat menstruasi?",
                          style: primaryTextStyle.copyWith(
                            fontSize: 14,
                            fontWeight: semiBold,
                          ),
          ...
          answerContainer("Berdzikir, sholawat dan berdoa", "A"),
          const SizedBox(height: 14,),
          answerContainer("Marah-marah", "B"),
        ],
      );
    }
