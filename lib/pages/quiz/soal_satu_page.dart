...
import 'package:smile_education/pages/quiz/soal_dua_page.dart';
import 'package:smile_education/theme/theme.dart';

import '../home_page.dart';

class SoalSatuPage extends StatefulWidget {
  const SoalSatuPage({super.key});

  @override
  State<SoalSatuPage> createState() => _SoalSatuPageState();
}

class _SoalSatuPageState extends State<SoalSatuPage> {
  String nama = '';
  String? selectedAnswer;
  final AudioPlayer _audioPlayer = AudioPlayer();

  // Fungsi untuk memutar audio
  void _playSoal1() async {
    await _audioPlayer.play(AssetSource('kuis/soal1.mp3'));
  }

  void _play1A() async {
    await _audioPlayer.play(AssetSource('kuis/1A.mp3'));
  }

  void _play1B() async {
    await _audioPlayer.play(AssetSource('kuis/1B.mp3'));
  }
  ...
  Future<void> _savePoints(int point) async {
    SharedPreferences prefs = await SharedPreferences.getInstance();
    await prefs.setInt('point1', point);
  }
  ...
  void _cekJawaban() {
    ...
    AwesomeDialog(
      ...
      btnOkOnPress: () {
        Navigator.push(
            context,
            MaterialPageRoute(builder: (context) => SoalDuaPage())
        );
      },
              ...
              children: [
                ...
                Padding(
                  ...
                  child: GestureDetector(
                    onTap: (){
                      if (label == "A") {
                        _play1A(); // Mainkan audio untuk jawaban A
                      } else if (label == "B") {
                        _play1B(); // Mainkan audio untuk jawaban B
                      }
                    },
                    child: Image.asset(
                      'assets/ic_sound.png',
    ...
    Widget soal(){
      return Column(
        children: [
                  ...
                  child: Row(
                    children: [
                      ...
                      Expanded(  // Wrap the Text widget with Expanded to allow it to take available space
                        child: Text(
                          "Salah satu tanda wanita telah baliq adalah?",
                          style: primaryTextStyle.copyWith(
                            fontSize: 14,
                            fontWeight: semiBold,
                          ),
          ...
          answerContainer("Kulit halus, cantik, tubuh semampai", "A"),
          const SizedBox(height: 14,),
          answerContainer("Menstruasi", "B"),
        ],
      );
    }

    return PopScope(
      canPop: false, // Mengizinkan back navigation
      onPopInvoked: ((didPop) {
        if (didPop) {
          // Ketika back button ditekan, tampilkan dialog konfirmasi
          return;
        }
