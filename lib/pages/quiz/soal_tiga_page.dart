...
import 'package:smile_education/pages/quiz/soal_empat_page.dart';
import 'package:smile_education/theme/theme.dart';

import '../home_page.dart';

class SoalTigaPage extends StatefulWidget {
  const SoalTigaPage({super.key});

  @override
  State<SoalTigaPage> createState() => _SoalTigaPageState();
}

class _SoalTigaPageState extends State<SoalTigaPage> {
  String nama = '';
  String totalPoin = '';
  ...
  void _playSoal3() async {
    await _audioPlayer.play(AssetSource('kuis/soal3.mp3'));
  }

  void _play3A() async {
    await _audioPlayer.play(AssetSource('kuis/3A.mp3'));
  }

  void _play3B() async {
    await _audioPlayer.play(AssetSource('kuis/3B.mp3'));
  }
  ...
  Future<void> _getTotalPoinFromSharedPrefs() async {
    ...
    setState(() {
      totalPoin = (point1+point2).toString();
    });
  }
  ...
  Future<void> _savePoints(int point) async {
    SharedPreferences prefs = await SharedPreferences.getInstance();
    await prefs.setInt('point3', point);
  }
  ...
  void _cekJawaban() {
    ...
    AwesomeDialog(
      ...
      btnOkOnPress: () {
        Navigator.push(
            context,
            MaterialPageRoute(builder: (context) => const SoalEmpatPage())
        );
      },
              ...
              children: [
                ...
                Padding(
                  ...
                  child: GestureDetector(
                    onTap: (){
                      if (label == "A") {
                        _play3A(); // Mainkan audio untuk jawaban A
                      } else if (label == "B") {
                        _play3B(); // Mainkan audio untuk jawaban B
                      }
                    },
                    child: Image.asset(
                      'assets/ic_sound.png',
    ...
    Widget soal(){
      return Column(
        children: [
                  ...
                  child: Row(
                    children: [
                      ...
                      Expanded(  // Wrap the Text widget with Expanded to allow it to take available space
                        child: Text(
                          "Bagaimana sikap anda jika melihat temanmu tembus pada saat mestruasi ?",
                          style: primaryTextStyle.copyWith(
                            fontSize: 14,
                            fontWeight: semiBold,
                          ),
          ...
          answerContainer("Acuh tak acuh", "A"),
          const SizedBox(height: 14,),
          answerContainer("Memberitahu dan membantu teman untuk menutupi roknya", "B"),
        ],
      );
    }
